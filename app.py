...
st.set_page_config(
    layout="wide", page_title="Real-Time Safety Detection", page_icon="🛡️"
)
...
def send_alert_email(incident_details):
    ...
    try:
        ...
        st.success(
            f"EMAIL ALERT SENT successfully to {RECIPIENT_EMAIL} for Incident {incident_details['id']}!"
        )
    except Exception as e:
        st.error(
            f"Simulated email failure: Could not connect to SMTP server. Error: {e}"
        )

    incident_details["notification_log"].append(
        {"time": datetime.now().strftime("%H:%M:%S"), "recipient": "Email Alert System"}
    )
...
class RealTimeDetector(VideoProcessorBase):
    ...
    def recv(self, frame):
        ...
        if self.violence_score >= 90 and not self.detection_triggered:
            ...
            if (
                "violation_detected" not in st.session_state
                or not st.session_state.violation_detected
            ):
            ...
            cv2.putText(
                img,
                f"Scanning... ({self.violence_score}%)",
                (20, 50),
                cv2.FONT_HERSHEY_SIMPLEX,
                1,
                (0, 255, 0),
...
def incident_list_panel():
    ...
    for incident in sorted_incidents:
        ...
        with st.container(border=False):
            button_label = (
                f"{incident['type']} at {incident['location']} ({incident['status']})"
            )

            st.markdown(
                f'<div class="incident-card-button" style="border-left: 5px solid {status_color.get(incident["status"], "#64748B")};">',
                unsafe_allow_html=True,
            )
            if st.button(
                button_label,
...
def send_notification_action(incident_id):
    for incident in st.session_state.incidents:
        if incident["id"] == incident_id:
            ...
            recipient = (
                "Admin/Security"
                if incident["notifications_sent"] == 1
                else "Parents/Counselors"
            )
            incident["notification_log"].append(
                {"time": datetime.now().strftime("%H:%M:%S"), "recipient": recipient}
            )
            st.toast(
                f"Notification sent to {recipient} for Incident {incident_id}!",
                icon="🔔",
            )
            break
    st.rerun()
...
def incident_detail_panel():
    ...
    if not incident_id:
        ...
        st.info(
            "Select an incident from the left panel to view details and manage the response."
        )
        return
    ...
    col3.metric(
        "Detection Confidence", f"{incident['deescalation_score']}%", delta="Confirmed"
    )
    ...
    with colA:
        ...
        st.selectbox(
            ...
            on_change=lambda: handle_update(
                incident_id, "status", st.session_state[f"status_{incident_id}"]
            ),
        )

    with colB:
        st.markdown(
            "<p style='font-weight: bold; margin-bottom: -5px;'>Immediate Notifications</p>",
            unsafe_allow_html=True,
        )
        st.button(
        ...
        if incident["notifications_sent"] > 0:
            st.caption(
                f"Last sent to {incident['notification_log'][-1]['recipient']} at {incident['notification_log'][-1]['time']}"
            )
    ...
    with colC:
        ...
        if incident["wellness_required"] > 0:
            st.button(
                "Launch Confidential Check-in Protocol", key=f"wellness_{incident_id}"
            )
    ...
    with colD:
        ...
        log_text = "\n".join(
            [
                f"**[{entry['time']}]** - Sent to {entry['recipient']}"
                for entry in incident["notification_log"]
            ]
        )
        if log_text:
        ...
        with st.container(border=True):
            ...
            with c2:
                action = (
                    ...
                    if zone["score"] > 90
                    else "Increase casual presence"
                )
                text_color_style = (
                    "color: #DC2626;" if zone["score"] > 90 else "color: #F59E0B;"
                )

                st.markdown(
                    f"**Recommendation:**\n<span style='{text_color_style} font-weight: bold;'>{action}</span>",
                    unsafe_allow_html=True,
                )
...
def process_uploaded_video(uploaded_file):
    ...
    if st.button("Run Full Video Detection Analysis", key="run_video_analysis"):
        ...
        with st.spinner(
            "Analyzing video footage for violent activity... This may take a few seconds..."
        ):
        ...
        if simulated_violence_score >= 85:
            st.error(
                f"🚨 **VIOLATION DETECTED in Video!** Confidence: {simulated_violence_score}%"
            )
            ...
            st.success(
                f"✅ No Significant Violation Detected in video. Confidence: {simulated_violence_score}%"
            )
...
def file_analysis_panel():
    ...
    st.caption(
        "Supports Image (.jpg, .png) and Video (.mp4, .mov) file uploads for retroactive analysis."
    )

    uploaded_file = st.file_uploader(
        "Upload a file", type=["mp4", "mov", "jpg", "jpeg", "png"]
    )
...
def main():
    ...
    st.caption(
        "Deep Learning integration with real-time alerts and incident management."
    )
    ...
    if (
        st.session_state.violation_detected
        and not st.session_state.email_sent_for_last_detection
    ):
        new_incident = generate_incident(
    ...
    with col2:
        tab1, tab2, tab3 = st.tabs(
            [" Real-Time Video Feed", " File Analysis", " Proactive Risk Management"]
        )

        with tab1:
            st.subheader("Live Deep Learning Feed")
            st.warning(
                "Note: Your camera will start automatically when this tab is selected."
            )
            ...
            if webrtc_ctx.video_processor:
                status = (
                    ...
                    else f"Simulated Score: {webrtc_ctx.video_processor.violence_score}%"
                )
